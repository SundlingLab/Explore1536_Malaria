...
subjectTable %>% 
  ...
  transmute(study_id,
            ...
            severe_5 = ifelse(severe_5==1,"yes","no"), 
            severity_lab) %>% 
  pivot_longer(cols = c(endemic,severe_5,severity_lab)) %>% 
  mutate(name = factor(name, levels = c("endemic","severe_5","severity_lab"))) %>% 
  mutate(value = factor(value, levels = c("previously_exposed","primary_infected","no","yes","severe","moderate","mild"))) %>% 
  ...
  ggplot(
    aes(x = name,
        ...
        alluvium = study_id,
        label = value))+
  geom_flow(aes(fill = severe_5)) +
  geom_stratum(aes(fill = value),width = .4) +
  scale_fill_manual(values = c(endemic2_col,c("yes"="tomato","no" = "grey80"),patient_kclust3_lab)) +
